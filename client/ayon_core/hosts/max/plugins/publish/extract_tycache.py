...
class ExtractTyCache(publish.Extractor):
    ...
    def process(self, instance):
        ...
        variant_name = instance.data.get("name")
        tyc_fnames = []
        tyc_mesh_fnames = []
        with maintained_selection():
            filename = f"{variant_name}.tyc"
            path = os.path.join(stagingdir, filename)
            filenames = self.get_files(
                variant_name, start_frame, end_frame)
            self._extract_tyflow_particles(
                operator, path, export_mode, material_cache)
            mesh_filename = f"{variant_name}__tyMesh.tyc"
        ...
        material_filename = f"{variant_name}__tyMtl.mat"
    ...
    def get_files(self, variant_name, start_frame, end_frame):
        ...
        for frame in range(int(start_frame), int(end_frame) + 1):
            filename = f"{variant_name}__tyPart_{frame:05}.tyc"
