"""Shared review-baking logic used by both the legacy dict-representation
extractor (`extract_review.py`) and the trait-based extractor
(`extract_review_traits.py`).

This module is a behavior-preserving extraction from `extract_review.py`.
`ReviewRenderer` and the module-level filtering helpers below were moved out
unchanged (aside from decoupling from `pyblish.api.InstancePlugin` / a
logger mixin) so both extractors run the exact same ffmpeg pipeline instead
...
DEFAULT_VIDEO_EXTS = {"mov", "mp4"}
DEFAULT_ALPHA_EXTS = {"exr", "png", "dpx"}
...
def input_is_sequence(repre: dict) -> bool:
...
# ---------------------------------------------------------------------------
# Layer A - instance-level profile/output selection.
# Pure instance/context data, no representation-shape coupling - shared
# unmodified by legacy and trait-based extractors.
# ---------------------------------------------------------------------------

def families_from_instance(instance) -> list[str]:
...
def families_filter_validation(families, output_families_filter) -> bool:
    """Determines if entered families intersect with families filters.

    All family values are lowered to avoid unexpected results.
    """
    families_filter_lower = {
        family.lower() for family in output_families_filter
    ...
    if not families_filter_lower:
        return True
    return any(
        family.lower() in families_filter_lower for family in families
    )


def filter_output_defs(profile, product_name, families) -> dict[str, Any]:
    ...
    for output_def in outputs:
        ...
        if not families_filter_validation(families, families_filters):
            continue
...
def get_profile_outputs_for_instance(instance, profiles, log) -> list[dict]:
    """Resolve settings profile -> list of output definitions for instance.

    Shared between legacy and trait-based review extractors: matches on
    host name / product base type / task type, then filters outputs by
    instance families and product name. Nothing here reads representation
    data, so behavior is guaranteed identical for both extractors.
    """
    ...
    instance_families = families_from_instance(instance)
    filtered_outputs = filter_output_defs(
...
def single_frame_filter(input_filepaths, output_defs) -> list[dict]:
    """Drop output defs whose `single_frame_filter` doesn't match input."""
...
def filter_outputs_by_custom_tags(outputs, custom_tags, log) -> list[dict]:
    ...
    Args:
        ...
        log: logger to write debug output to.
    ...
    log.debug("__ filtered_outputs: {}".format(
...
def review_repre_tag_filter(tags: list[str]) -> tuple[bool, Optional[str]]:
    ...
    Same review/thumbnail/passing semantics used by the legacy extractor's
    `_get_outputs_per_representations`, taking a plain tag list so it can be
    fed either `repre.get("tags")` (legacy) or `Tagged.tags` (trait-based) -
    the two extractors agree on what "review"/"thumbnail"/"passing" mean.

    Returns:
        tuple[bool, Optional[str]]: (should_process, skip_reason)
    """
    if "review" not in tags:
        return False, "Didn't find \"review\" in tags"
    if "thumbnail" in tags:
        return False, "Found \"thumbnail\" in tags"
    if "passing" in tags:
        return False, "Found \"passing\" in tags"
    return True, None


# ---------------------------------------------------------------------------
# ReviewRenderer - the ffmpeg pipeline itself. Operates on a plain repre
# dict in/out (same shape as the legacy `new_repre`). No pyblish/instance
# mutation beyond reading instance.data - callers decide what to do with the
# returned representation dicts (append to instance.data["representations"]
# for legacy, convert to trait Representation for the trait extractor).
# ---------------------------------------------------------------------------

class ReviewRenderer:
    """Runs the review-baking ffmpeg pipeline for output definitions.

    Lifted from `ExtractReview` verbatim; only entry/exit points changed
    from "mutate instance.data in place" to "return new repre dicts".
    """
    ...
    def __init__(
        ...
        self.log = log
        self.image_exts = image_exts or set(DEFAULT_IMAGE_EXTS)
        self.video_exts = video_exts or set(DEFAULT_VIDEO_EXTS)
        self.supported_exts = self.image_exts | self.video_exts
        self.alpha_exts = alpha_exts or set(DEFAULT_ALPHA_EXTS)
    ...
    def render_repre_outputs(
        ...
        """Render all `output_defs` for one source `repre` dict.

        This is the per-representation body that used to live inline in
        `ExtractReview.main_process` (OIIO pre-conversion handling) plus
        `_render_output_definitions` (per-output ffmpeg baking), merged and
        made to return results instead of mutating `instance.data`.

        Returns:
            list[dict]: newly created representation dicts. Empty if this
                representation was skipped (e.g. missing files, conversion
                could not be determined).
        """
        # Local import to avoid a hard dependency for callers that only
        # need the filtering helpers above.
        from ayon_core.lib.transcoding import (
        ...
        input_filepaths = []
        if not input_is_sequence(repre):
            ...
            for filename in repre["files"]:
                filepath = os.path.join(src_repre_staging_dir, filename)
                input_filepaths.append(filepath)
                if first_input_path is None:
                    first_input_path = filepath

        filtered_output_defs = single_frame_filter(
        ...
        try:
            ...
            if do_convert:
                ...
                if new_staging_dir and os.path.exists(new_staging_dir):
                    shutil.rmtree(new_staging_dir)

    # -- ffmpeg pipeline (unchanged from ExtractReview) ----------------
    ...
    def _render_outputs(
        ...
        for _output_def in output_definitions:
            ...
            self.log.debug(
                "Rendered new representation: {}".format(new_repre)
            )
            new_repres.append(new_repre)
    ...
    def prepare_temp_data(self, instance, repre, output_def) -> TempData:
        ...
        input_is_seq = input_is_sequence(repre)
        input_allow_bg = False
        first_sequence_frame = None

        if input_is_seq and repre["files"]:
        ...
        return TempData(
            ...
            origin_repre=repre,
            input_is_sequence=input_is_seq,
    ...
    def input_output_paths(self, new_repre, output_def, temp_data: TempData):
        """Deduce input and output file paths based on entered data.
...
# ---------------------------------------------------------------------------
# Overscan helpers (unchanged, moved verbatim).
# ---------------------------------------------------------------------------
