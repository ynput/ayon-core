...
def _load_rv_stylesheet(old: bool = True) -> str:
    filename = "rv_mac_dark_legacy.qss" if old else "rv_dark.qss"
    fpath = CURRENT_DIR / filename
...
def _read_json_file(fpath: Path):
...
def get_test_project_data() -> ProjectData:
    # read project data
    project_file = CURRENT_DIR / "sample_project_data.json"
...
def get_test_activity_data() -> ActivityData:
    activity_file = CURRENT_DIR / "sample_activities.json"
    activity_data = _read_json_file(activity_file)
...
def _load_statuses() -> list[dict[str, Any]]:
    statuses_file = CURRENT_DIR / "sample_statuses.json"
    statuses_data = _read_json_file(statuses_file)
